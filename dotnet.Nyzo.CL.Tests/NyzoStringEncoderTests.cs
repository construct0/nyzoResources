...
public class NyzoStringEncoderTests {
	...
	public void DecodeToContentBytes_ShouldReturnBytes_OrNull() {
		...
		//	// Get the bytes
		//	var txBytesWithSignature = tx.GetBytes(true);
	...
	// TODO: incorporate GetByteSize() and FieldByteSize from nyzoVerifier.NyzoTransaction (decode method needs content size, needs to be added to GetBytes)
	//[Fact]
	...
	//	// Invalid, character that does not exist in character map
	//	var badUnsupportedCharPrefilledData = firstVectorEncodedPrefilledData.Replace("e", "й");

	//	Assert.Throws<KeyNotFoundException>(() => ShimNyzoStringEncoder.DecodePrefilledData(badUnsupportedCharPrefilledData));
	...
	//	Assert.Null(ShimNyzoStringEncoder.DecodeNyzoTransaction(badOnlyFirstHalftx));
